  ...
  public function data() {
    ...
    foreach ($results as $result) {
      $rows[] = [
        ...
        $result->birthdate,
        ($result->position == 'admin') ?$this> t ('Administrador'): (($result -> position == 'webmaster') ?$this> t ('Webmaster'): (($result -> position == 'developer') ?$this> t ('Desarrollador'): '')),
        ($result -> status ==1) ?$this> t ('Activo'): (($result -> status ==0) ?$this> t ('Inactivo'): ''),
      ];
    }
    $output = [
      '#theme' => 'table',
